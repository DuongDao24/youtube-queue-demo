    ...
    if os.path.exists(CONFIG_PATH):
        try:
            ...
            if isinstance(data, dict):
                ...
                config["nick_change_hours"] = int(data.get("nick_change_hours", 24))
                config["names"]             = data.get("names", {})
                config["name_changed_at"]   = data.get("name_changed_at", {})
        except Exception as e:
...
def api_add():
    ...
    ip   = client_ip()
    name = (data.get("name") or "").strip() or config["names"].get(ip)
...
def api_play():
    ...
    if vid:
        if current: history.appendleft(current)
        title = fetch_title(vid)
        current = {"id": vid, "title": title, "by_ip": "host", "by_name": "Host", "ts": int(time.time())}
        save_state()
    else:
        if not current:
            set_next_current()
