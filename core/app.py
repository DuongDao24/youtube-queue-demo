...
CHANGELOG (v01.6.2):
- Persistent storage in /core/data (JSON + CSV) so state survives restarts.
- Queue/History auto-trim (Queue<=50, History<=20). Auto-clear is DISABLED by decision.
- CSV activity logging for submit/played/clear/setting changes.
- Nickname cooldown truly enforced: when on cooldown, submissions keep using the LAST allowed nickname.
- Backend moved into /core; IMPORTANT: this app is configured to use templates/static at repo ROOT.

NOTES:
- If you deploy on Render or similar, set start command: "python core/app.py"
"""
...
# IMPORTANT: point Flask to root-level templates/static
...
default_settings = {
    "system": {
        ...
        "auto_clear_days": 7,   # configured but NOT enforced (disabled block below)
        "theme": "dark",
        "version": "v01.6.2"
    },
    "branding": {
...
# To enable in the future, uncomment the block below.
# try:
...
# -----------------------------
# Flask app (point to ROOT templates/static)
# -----------------------------
